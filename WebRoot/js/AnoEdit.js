$(document).on("click", "#submitTopic", function () {
    $.ajax({
               ...
               success: function (data) {
                   if (data.status == true) {
                       var isValidAnoTopic = false;
                       var isValidAnoContent = false;
                       var anoTopic = $("#anoTopic").val();
                       var anoContent = $("#anoContent").val();
                       if (anoTopic.length < 1) {
                           $("#anoTopic-msg").html("主题不能为空！")
                       } else {
                           isValidAnoTopic = true;
                       }

                       if (anoContent.length < 1) {
                           $("#anoContent-msg").html("描述不能为空！")
                       } else {
                           isValidAnoContent = true;
                       }

                       if (isValidAnoTopic && isValidAnoContent) {
               ...
               success: function (data) {
                   if (data.status == true) {
                       ...
                       $("#getUserID").html('<li><a>' + userAccount + '</a></li>'
                                            + '<li><a href="Ano_edit.html" id="postMeg">发帖</a></li>');
                       $("#getUserID").toggle();
                   } else {
                       $("#getUserID").toggle();
                   }
                   ...
                   $.each(data.postList, function (i, item) {
                       str +=
                           ('<li class="list-group-item"><a class="pageGetName" style="color: #000000;" href="#" id="getName'
                            + i + '">' + item.post_title + '<p id="getId' + i + '" hidden>'
                            + item.post_id + '</p></a></li>');
                   });
...
$(document).on("click", ".pageGetName", function () {
    ...
    $.ajax({
               ...
               success: function (data) {
                   ...
                   $("#remove2").remove()
                   $("#remove3").remove()
                   var title = ('<div class="col-md-12 col-sm-12 text-center"><h2>' + topicTitle
                                + '</h2></div>')
                   ...
                   $.each(data.comments, function (i, item) {
                       content +=
                           ('<li class="list-group-item">' + item.comment_content
                            + '<h2 class="text-right"><small>匿名用户' + item.comment_time
                            + '</small></h2><p hidden>' + item.user_account + '</p>' + '</li>'
                            + '</li>')
                   });
                                          ...
                                          if (data.status == true) {
                                              ...
                                              $.ajax({
                                                         ...
                                                         success: function (data) {
                                                             ...
                                                             $("#remove2").remove()
                                                             $("#remove3").remove()
                                                             var title = ('<div class="col-md-12 col-sm-12 text-center"><h2>'
                                                                          + topicTitle
                                                                          + '</h2></div>')
                                                             var str2 = ('</ul></div></div></div><div class="col-sm-2 col-md-2"></div></div>')
                                                             var str = ('<div class="row Czb_margin_padding"><div class="col-sm-2 col-md-2"></div><div class="col-sm-8 col-md-8" ><div class="thumbnail"><div class="caption"><ul class="list-group" >')
                                                                    ...
                                                                    function (i, item) {
                                                                        content +=
                                                                            ('<li class="list-group-item">'
                                                                             ...
                                                                             + '<h2 class="text-right"><small>匿名用户'
                                                                             + item.comment_time
                                                                             + '</small></h2><p hidden>'
                                                                             + item.user_account
                                                                             + '</p>' + '</li>'
